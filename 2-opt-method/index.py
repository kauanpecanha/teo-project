...
class HGS_MDVRP:
    ...
    def local_search_2opt(self, tour):
        best_tour = list(tour)
        best_fit = self.avaliar(best_tour)
        improved = True
        
        # Executa até parar de melhorar (Hill Climbing)
        # Ou limite com um contador para não ficar lento demais
        count = 0
        while improved and count < 5:  # Limite de passes para não travar
            improved = False
            count += 1
            for i in range(1, len(tour) - 2):
                for j in range(i + 1, len(tour)):
                    if j - i == 1: continue # Trocas adjacentes não mudam distâncias em 2-opt simples
                    
                    # Cria novo tour invertendo o segmento entre i e j
                    new_tour = best_tour[:i] + best_tour[i:j][::-1] + best_tour[j:]
                    
                    # Avaliação Delta seria ideal, mas avaliar tudo funciona
                    fit = self.avaliar(new_tour)
                    
                    if fit < best_fit:
                        best_fit = fit
                        best_tour = new_tour
                        improved = True
                        break # Reinicia a busca na primeira melhora (First Improvement)
                if improved: break
        ...
        for g in range(self.geracoes):
            ...
            while len(new_pop) < self.pop_size:
                ...
                if random.random() < 0.6: 
                    child_seq, fit = self.local_search_2opt(child_seq)
                else:
